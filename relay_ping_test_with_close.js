...
if (path.resolve(process.argv[1]) === __filename) {
  (async () => {
    ...
    let pingTimeout;
    let pongReceived = true; // Start with true to not fail on first ping
    ...
    if (ws) {
      ...
      ws.on('close', (code, reason) => {
        ...
        console.log(`\nWebSocket connection to ${relayUrl} closed. Code: ${code}, Reason: ${reasonString || '(empty)'}`);
      });

      const sendPing = () => {
        console.log('Connection statuses:', pool.listConnectionStatus());

        if (!pongReceived) {
          console.log(`\nNo pong received from ${relayUrl} in time. Closing connection.`);
